...
| KB-ANTI-LAZY-001 | 禁止用正则猜参数——卖家精灵详细参数有就不用猜 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-002 | 禁止竞对参数不验证——竞对必须Amazon详情页验证 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-003 | 禁止场景全填"通用"——使用场景必须从标题精确提取 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-004 | 禁止字段只提取2-3个——必须穷举维度 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-005 | 禁止结论空泛——必须有统计数据支撑 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-006 | 禁止不打包.zip直接发.xlsx——Discord收不到 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-007 | 禁止子代理context不够硬上——关键步骤亲自做 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-008 | 禁止只列数据不下判断——分析必须有观点 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-009 | 禁止用browser_navigate代替卖家精灵——BSR懒加载只渲染30个 | 11 | 规则 | pending（待审核） |
| KB-ANTI-LAZY-010 | 禁止数据来源不具体——不能只写"网络""亚马逊" | 11 | 规则 | pending（待审核） |

**类型统计**：规则40条 / 知识5条 / 失败案例4条 / 成功案例2条 / 防偷懒10条 | 合计54条
