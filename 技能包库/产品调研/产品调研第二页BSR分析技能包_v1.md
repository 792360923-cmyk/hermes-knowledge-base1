...
创建日期：2026-08-25
更新日期：2026-08-25
适用场景：Amazon BSR前100功能与数据分析（第2页）
来源：历史经验/Hermes技能包
是否当前推荐：否
上一版本：无
下一版本：无
维护人：Hermes小助理
审核人：未审核
文件路径：技能包库/产品调研/产品调研第二页BSR分析技能包_v1.md
---
...
## 固定板块（17列，始终包含）
小类排名序号、图片(60×60嵌入)、中文标题、品牌、大类目排名、价格、评分、评论数、上架年份时间、ASIN、链接(可点击)、FBA运费、配送渠道(FBA/FBM)、月销量、月销售额美金、月销售额RMB、竞品类型
...
| 数据行 | 微软雅黑 9pt，行高55-75px |
| 竞对行 | 红底 #FFC7CE 加粗 |
| 类型分底色 | 按竞品类型分色（如插电大空间浅蓝DDEBF7/便携电池浅黄FFF2CC/车用浅绿E2EFDA，按实际类目自定） |
| 功能/差异化卖点列 | 红字 #FF0000 加粗 |
| 图片 | 60×60px 缩略图嵌入图片列 |
...
## 技术细节
- **主图下载**：从BSR页或产品页提取主图ID（`#landingImage` src 的 `images/I/{ID}`），用 `curl https://m.media-amazon.com/images/I/{ID}._AC_UL320_.jpg` 下载，openpyxl 嵌入 60×60。图片ID含`+`号的文件名须去`+`。
