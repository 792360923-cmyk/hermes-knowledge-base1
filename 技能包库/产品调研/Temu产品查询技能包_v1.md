...
维护人：Hermes小助理
审核人：未审核
文件路径：技能包库/产品调研/Temu产品查询技能包_v1.md
---
...
Calls the EHunt data source via the Nexscope proxy gateway, route **`/api/v1/tools/research/ehunt/temu/productQuery`** (display name: **Temu Product Query**). Authentication and routing are handled by the Nexscope proxy; if the response contains a root-level `code` field, success is determined by the live response.
